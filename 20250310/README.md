# 데이터 베이스(DB)
> 정보를 규칙적 체계적으로 저장 관리하기 위한 시스템
> 소프트웨어를 개발할 때 필요한 데이터를 효율적으로 관리하기 위해서 사용하는 것
> 쇼핑몰을 예를 들면 사용자 계정 정보, 신상정보 거래 내용 등등을 안전하게 보관하기 위해서

## 데이터베이스의 장점
> 이전에는 문서로 데이터를 기록하고 관리를 했는데, 문서로 관리하는 방법은 관리도 어렵고 원하는 데이터를 찾기가 힘들다. 검색도 힘들고 분실의 위험도 있었다. 컴퓨터가 발전을 하면서 빠르게 데이터를 관리할 수 있는 데이터베이스를 개발하여 사용하게 되었다.
...
1. 파일 시스템
> 초기에는 데이터 저장 방식은 파일 시스템이었다. 개별 파일에 데이터를 저장해서 보관, 메모장 처럼 값을 입력해서 파일의 형태로 저장

- 같은 데이터가 중복될 가능성이 있음
- 데이터를 검색할 때 비효율적이고 수정할 때 모든 파일을 확인해서 수정해야한다.

2. 계층형 데이터 베이스
> 파일 시스템의 문제인 데이터를 수정할 때, 값을 조회하는 부분이
> 계층형 IBM에서 개발한 데이터 베이스, 트리 구조를 사용해서 데이터의 부모와 자식 관계로 저장하는 방식
> 여러 부모와 자식 관계를 허용해서 데이터를 관리하는 방식

- 데이터의 구조가 복잡해짐, 데이터의 수정도 복잡하기 때문에 관리하기가 어렵다.

3. 관계형 데이터 베이스의 개념이 탄생
> 관계형 데이터 베이스의 개념이 탄생
> 데이터를 표의 형태로 저장해서 SQL이라는 구문을 사용해서 데이터를 쉽게 조회하고 저장할 수 있도록 설계
...
4. NoSQL 데이터베이스
> 데이터가 너무 대량으로 방대해지다 보니, 대량의 데이터를 처리하는데 한계가 있었다. 확장성이 좋은 형태로 NoSQL의 데이터베이스가 탄생

- 키와 값으로 데이터를 저장, 문서 기반으로 저장하는 저장소 열 기반 데이터 저장소
- 자유롭다보니 규칙을 작성해서 체계적으로 사용해야한다.

> 데이터의 값의 자주 변경이 필요한 경우 유용하다

### 데이터베이스 관리 시스템 (DBMS)
> 데이터 베이스를 생성하고 관리하는 소프트웨어
> 소프트웨어가 하는 역할은 데이터의 저장과 검색, 수정, 삭제
> 보안과 권한을 관리

### SQL(Strutured Query Language)
> SQL은 관계형 데이터 베이스의 데이터를 제어하는 언어

1. DDL(Data Definition Language) 데이터의 정의어 : create(생성), alter(수정), drop(삭제) -> 테이블을 조작
> 테이블의 관계의 구조를 생성하는데 사용하는 구문
> 테이블의 생성, 수정, 삭제

2. DML(Data Manipulation Language) 데이터 조작어 : INSERT, UPDATE , DELETE(삭제) -> 데이터의 값을 조작
> 테이블의 데이터 검색 생성, 수정, 삭제 (CRUD)

3. DQL(Data Query Language) 데이터 조회어 : SELETE (조회)
> 테이블의 값을 검색할 때 사용

4. DCL(Data Control Language) 데이터 제어어 : GRANT REVOKE -> 사용자의 권환을 제어
> 데이터의 사용 권환을 관리할 때 사용
...
- 고립성 (lsolation) : 여러 트랜잭션이 독립적으로 실행된다.
- 지속성 (Durability) : 트랜잭션이 완료되면 영구적으로 데이터를 저장한다.

### MySQL
> 오픈소스로 많이 제공되고 있고 무료 데이터베이스
...
# 시스템 환경변수로 지정
# 돋보기 누르고 시스템 환경변수 편집 열고
# 환경변수 버튼 누르고
# 유저에 대한 사용자 변수에 Path 클릭 새로추가
#  C:\Program Files\MySQL\MySQL Server 8.0\bin 경로를 넣고 확인
...
1. colum(열) : 테이블의 세로 항목 데이터의 유형을 표현
2. row(열) : 테이블의 가로 항목의 컬럼의 값들을 하나의 로우로 구성
