...
document.onclick =  function(e) {
    if(e.target.classList.contains("increase")){
      count++;
      value.innerHTML = count; // 텍스트가 화면에 보이도록
      ...
      if (count > 10) {
        value.style.color = "#000069"; 
      } else if (count === 0) {
        value.style.color = "black";  
      }


function animationBtn(button, animation) { 
    button.classList.add(animation); 
    setTimeout(() => {
      button.classList.remove(animation); // animation 제거 
    }, 400); // 4초뒤
}
